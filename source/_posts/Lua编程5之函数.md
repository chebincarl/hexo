...
目前已经可以写大部分的基础代码了，但是编程中最重要的就是如何提高代码的复用，最基础的方法就是提取函数。今天就来看看Lua中如何编写函数。
...
<span style="color:blue">optional_function_scope</span>: 该参数是可选的制定函数是全局函数还是局部函数，未设置该参数默认为全局函数，如果你需要设置函数为局部函数需要使用关键字 local。
