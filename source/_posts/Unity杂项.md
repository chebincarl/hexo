...
3D艺术往往都是由三大元素构成的：顶点（Vertex）、边（Edge）和面（Face）。顶点是指3D空间中的点。Cube有8个顶点（或者叫点）--每个顶角上各有一个。在下图中，用Cube的网格来解释。

边将这些点连在一起--在顶点之间形成线条。Cube有12条边：4条边位于上方，4条边位于下方，还有4条边位于每个顶角上，将顶部和底部连接起来，如下图所示。
