...
将其重命名为MonsterAnim，然后双击，IDE显示Animator视图，如下图所示。Animator视图中，可以像使用画图标的工具一样使用WYSIWYG方式拖曳动画片段，以设置Mecanim动画。按下快捷键（Windows: Alt键）并用鼠标拖曳，可以上/下、左/右移动该设计区域。
...
请注意，如果想在Animator视图中查看动画执行进度条，需要先在Hierarchy视图中选定怪兽模型。下面添加怪手走路的动画片段，直接拖曳walk动画片段到Animator图中即可，如下图所示。
...
怪兽状态有休眠、追击、攻击、死亡这4种。怪兽生成时执行休眼动画，玩家接近并进入追击范围时，怪兽开始进入追击状态，到达可攻击的范围内则执行攻击动画。因此，需要测量怪兽和玩家之间的距离，并定期更新怪兽状态，使其执行相应动画。

如下图所示，编写MonsterCtrl脚本。

怪兽状态根据其与玩家之间距离的变化而变化，即需要判断当前距离是否属于攻击范围，还要根据怪兽状态将执行相应动作的函数变为二元函数。 Start函数中，可以将一开始就追击玩家的逻辑删除或者为其添加注释。
//设置要追击对象的位置后，怪兽会马上开始追击。nvngent. deatination -playezTr.position;
如脚本6-2所示，修改MonsteCrl脚本。
脚本6-2 MonsterCtr:实现FSM

脚本中定义了一个Enum (枚举型)变量，检视视图中也以下拉框形式标注了该枚举型变量，如

图6-33所示。编写程序时，使用这种Enum类型的变量要比使用int， string等类型的变量的可读性更好，更有助于理解代码。
//声明表示怪兽状态信息的Enumerable变量
public enum Monaterstate (idle， trace， attack， die }，//保存怪鲁当前状态的Enu变量
Public Monaterscate monaterstate - MonaterState.ldle;


始终检查怪普状态的逻辑使用了协程函数，当然，也可以在Update()函数中实现同样的逻辑。但与其在每一帧都调用Update()函数查看怪兽状态，不如使用例程，以更加适当的周期间隔进行操作2更加高效，也能使代码更简洁。

CheckMonsterState()函数以0.2秒为间隔反复测量玩家与怪兽之间的距离，然后更新怪普状态这个函数添加了一个whil语句isDic，换言之，只要怪兽活着，下列循环就会一直执行;而如果怪作死掉，当然就无需更新状态变量。

其中， yield return new MaitForSeconds (0.2E)这行代码可以使程序逻辑暂时挂起，其传人的参数即为挂起事件，单位为秒(see)。也就是说， yield return语句能够实现逻辑待机。可以将之理解为其他编程语言的Sleep功能，一般在需要等待一定时间间隔时使用。
//等待0.2秒后再执行后续代码
yield return new waltForSeconds (0.2)，

玩家接近怪兽时，怪兽需要选择追击还是攻击，这就需要计算两个游戏对象之间的距离。Vector3Distance(Vector3 a， Vector3 b)函数测量并返回a， b之间的距离， MonsterCtrl脚本向该函数传入2个参数，分别是玩家当前位置playerTr.position和怪兽当前位置monsterTr.position.
//测量怪兽与玩家之间的距离
float diat . Vector3.piatance (plaverTr.position， monsterTr.position);

MonsterAction()函数可以在变量isDie为true时跳出循环，并根据monsterState变量值的不同而利用Switch语句产生分支。

如果将场景视图中的Monster与Player事先分开一定距离(大于追击范围)，那么游戏运行开始，怪兽会是idle状态。如果使Monster靠近Player，游戏一开始怪兽便会追击。如果重新将Player置于较远距离以模拟跑出追击范围的场景，则怪兽会停止而不再追击。游戏运行过程中，可以查看检视视图中的monsterState怪兽状态值的变化。

怪兽状态为MonsterState.idle时，应该停止追击，进入idle状态。脚本中的NavMeshAgent.Stop()函数即可实现停止追击。
//ldle状态case Monaterstate.idle:1/停止追击nvigent.Stop();break，
MinswdteneaeeAla河:
3oene

玩家重新进人怪兽追击范围内，怪兽状态值会变为MonsterState.trace，此时通过NavMeshAgentdestination设置玩家当前位置值，以使怪善开始追击。NavMeshAgent.Resume( )方法可以重启处于停止状态的NavMeshAgent，
...
脚本6-3 MonsterCtrd:分配Animator组件并修改MonsterAction函数

使用表6-4中的函数可以访问Animator视图中显示的各种类型的变量。表6-4访问Animator中变量的函数
Set函数
Animator. SetBoolAnimator. SetFloatAnimator. SetintegerAnimator. SetTrigger
Get函数
Animator.GetBooAnimator.GetFloatAnimator.GetintegerAnimator.GetTrigger
