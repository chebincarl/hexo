...
下面为Floor材质指定纹理。选定该材质，在Inspector视图中查看其属性。如下图所示，选定Floor之后，将纹理图像文件夹中的Metal textures pack/patter 10/diffuse纹理拖曳到Inspector视图中的Albedo插槽、将Normal纹理拖曳到Normal Map插槽即可。设置完成后，可以通过底部预览区域查看Sphere模型附上纹理之后的效果。
