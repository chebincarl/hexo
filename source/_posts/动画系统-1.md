...
动画系统基于一种名为Animation Clip（动画剪辑）的资源，这些资源以文件的形式存在工程中。这些文件内的数据记录了物体如何随着时间移动、旋转、缩放，物体上的属性如何随着时间变化等。每一个Clip文件是一段动画。
...
<span style="color:red;">hift + K</span> Key All Modified，将动画属性列表中所有已修改的属性的数值记录为关键帧。
