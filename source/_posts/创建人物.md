...
首先在Project视图点击鼠标右键“Create”->“Avatar Mask”按钮，创建一个LayerMask层级遮罩，并设置为图1中的样式。视图中红色的部分是遮罩部分，绿色部分是正常部分。表明一旦某个Layer使用了LayerMask，该Layer下的所有动画只作用于绿色部分，也就是这里的右手。
...
如图2所示，建立从Empty至Wave的转换连线，该连线的设置如图3所示，再建立从Wave至Empty的转换连线，连线的设置如图4所示。
